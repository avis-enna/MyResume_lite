...
export default function AdminDashboard() {
  ...
  const checkAuth = async () => {
    try {
      ...
      console.log('DASHBOARD: Auth data received:', data);

      if (data.authenticated) {
        console.log('DASHBOARD: Auth successful, setting authenticated to true');
        setIsAuthenticated(true);
      } else {
        console.log('DASHBOARD: Auth failed, redirecting to /admin');
        router.push('/admin');
      }
    } catch (error) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
            ...
            <p className="text-blue-300">6 Sections Available</p>
          </div>
        </div>
      </main>
    </div>

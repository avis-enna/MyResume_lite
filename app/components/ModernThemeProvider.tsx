...
export function ModernThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const toggleDarkMode = () => {
    ...
    if (newMode) {
      document.documentElement.classList.add('dark');
      localStorage.setItem('modern-theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('modern-theme', 'light');
    }
  };

  return (
    <ModernThemeContext.Provider value={{ isDarkMode, toggleDarkMode }}>
      {children}
...
export function ModernThemeToggle() {
  const { isDarkMode, toggleDarkMode } = useModernTheme();
